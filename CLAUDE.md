...
**Phases 1, 2, 3, 4, 5, and 6 are built and green-lit (Phases 1–2 on 2026-05-29; Phases 3–6 on 2026-05-30).** Phase 4's offline logic green-light passed (16/16) and it ran clean against live data + DB in dry-run. Phase 5's offline logic green-light passed (24/24) and the watchlist filters ran against live data + DB (admin tool + dry-run cycle). Phase 6's offline logic green-light passed (21/21); it was built **offline-only** (no Anthropic key wired this session), so the AI layer is neutral (pure TA) until a key is provided. News source = the Alpaca news API; default model `claude-opus-4-8` (tunable via `ai.model`). Carry-overs needing a LIVE event to close: a Phase 4 paper liquidation that writes `trade_history`, a Phase 5 filter REJECTION / drift force-exit in a real cycle, and the Phase 6 live AI run (apply `db/06_schema_phase6.sql`, wire a key, toggle `ai_enabled`) — see TODO.md. The repo holds working code plus the design docs:
...
Strategy parameters (indicator periods, confidence weights, S/R lookback, breakout/volume rules, cooldown, TP multiple, order mechanics, Position-Manager trailing, watchlist filters, AI layer) live in `app_config` under the `ta.*`, `sr.*`, `breakout.*`, `weights.*`, `scan.*`, `cooldown.*`, `tp.*`, `execution.*`, `pm.*`, `filters.*`, `ai.*` keys (plus the master `ai_enabled` toggle) — **tunable in SQL without code changes** (read via `core/config.py`). The risk *envelope* (max/min risk %, confidence floor, portfolio ceiling, max open positions) lives in the `risk_profile` table, not `app_config`. See the "Defaults chosen" block in `TODO.md` Phase 2.

**Phases 7–8 are not built yet.** The `heartbeat` table they call for **does not exist** — search/verify before referencing it. `trade_history` is written by the Phase 4 Position Manager on position close, so it stays EMPTY until a position actually closes (pos#1 is still open on paper as of the Phase 4 green-light — and roughly flat, so nothing has tripped a trigger yet).
