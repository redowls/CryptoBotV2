...
**Phases 1, 2, and 3 are built and green-lit (Phases 1–2 on 2026-05-29; Phase 3 on 2026-05-30).** The repo holds working code plus the design docs:
...
- `core/` — `crypto.py` (Fernet), `db.py` (SQLAlchemy + pyodbc, retry, `get_active_credentials`), `config.py` (typed `app_config` accessor), `indicators.py` (pure EMA/RSI/ATR/vol-z), `levels.py` (rolling-extrema S/R + persist/read), `market_data.py` (closed-bars-only OHLCV fetch w/ Alpaca retry, `market_bars` upsert, `timeframe_duration`), `ta_engine.py` (the gatekeeper: `evaluate()` → signal + confidence + breakout boost), `sizing.py` (locked size formula + clamps), `execution.py` (the `ta_signal` gate + paper order + fill recording), `orchestrator.py` (the hourly entry cycle).
- `db/01_schema_phase1.sql`, `db/02_schema_phase2.sql`, `db/03_schema_phase3.sql` — **all three already applied** to the live `CryptoBotV2` DB. (Phase 3 is a config seed only — no new tables.)
- `scripts/` — `insert_credentials.py` (you run it; stores encrypted keys), `check_connectivity.py` (Phase 1 green-light), `run_signal_scan.py` (Phase 2 manual scan; persists signals + S/R, places no orders), `run_trade_cycle.py` (Phase 3 manual entry cycle; `--dry-run`, `--force`), `apply_sql.py` (GO-splitting `.sql` loader).
- `requirements.txt`, `.env.example`, `.gitignore`, `README.md`, and a local `.venv/` (Windows dev, deps installed).

Strategy parameters (indicator periods, confidence weights, S/R lookback, breakout/volume rules, cooldown, TP multiple, order mechanics) live in `app_config` under the `ta.*`, `sr.*`, `breakout.*`, `weights.*`, `scan.*`, `cooldown.*`, `tp.*`, `execution.*` keys — **tunable in SQL without code changes** (read via `core/config.py`). The risk *envelope* (max/min risk %, confidence floor, portfolio ceiling, max open positions) lives in the `risk_profile` table, not `app_config`. See the "Defaults chosen" block in `TODO.md` Phase 2.

**Phases 4–8 are not built yet.** The modules they call for (`core/position_manager.py`, `ai_*.py`) and the `heartbeat` table **do not exist** — search/verify before referencing them. `trade_history` is *declared* (Phase 1) and *written* only on position close, which is Phase 4 — so it stays empty until then; the orchestrator's exit pass is a marked Phase 4 placeholder.
...
- **`core/execution.py`** — `make_trading_client(key, secret, *, paper=True)`; **`place_entry(client, *, ta_signal, symbol, qty, time_in_force="gtc", poll_attempts=5, poll_delay=1.0) -> Fill`** — raises `PermissionError` unless `ta_signal is True` (invariant #1, structural), submits a market BUY, polls until the order reaches a terminal state, and returns a `Fill(order_id, symbol, requested_qty, filled_qty, avg_price, status)` with `.is_filled`. Broker I/O is kept OUT of the DB transaction. `record_position(conn, *, symbol, fill, sl_price, support_break_price, tp_price, confidence, breakout_confirmed, risk_pct_used) -> int` writes the open `positions` row from the ACTUAL fill (invariant #7) with both stop triggers (invariant #5); returns the new id.
- **`core/orchestrator.py`** — `run_cycle(*, symbols=None, dry_run=False, force=False) -> int`: one hourly cycle. Per symbol: fetch closed bars → upsert → detect+persist S/R → `evaluate` → persist signal; then the ENTRY pass (skip if held / no signal / below floor / in cool-down → `compute_size` → `place_entry` → `record_position`). Tracks open-position count, aggregate open risk, and cash *across* the cycle so later symbols respect capital committed by earlier ones. `dry_run` sizes + reports but places no orders; `force` (DEV) synthesizes a BUY setup (keeps real hints) to exercise an entry. TP = `entry_hint + tp.r_multiple * (entry_hint − stop_hint)`. The EXIT pass is a Phase 4 placeholder.

**Phase 4 wiring:** open positions carry `sl_price`, `support_break_price`, `tp_price`, `confidence_at_entry`, `breakout_confirmed`, `risk_pct_used`, `alpaca_order_id`. The Position Manager reads persisted `sr_levels` (via `levels.latest_levels`, invariant #6), enforces first-to-fire of the two stops (invariant #5), trails the SL only in favor (invariant #3), expands TP, and writes `trade_history` with the correct `exit_reason` on close. The orchestrator's `cooldown.bars_after_stop` check already reads `trade_history`, so it activates automatically once Phase 4 records stop-outs.

## Phase discipline

Work strictly top-down through `TODO.md`. **No phase begins until the previous phase passes its green-light test.** Each phase has an explicit test checklist in `TODO.md`. Phase 1's green-light test (`scripts/check_connectivity.py`) shows four PASS lines: DB reachable, Alpaca creds decrypt, paper account returns equity, live quote returns bid/ask. **Phase 1 passed all four on 2026-05-29. Phase 2 (Market Data + TA Engine) also passed on 2026-05-29 (`scripts/run_signal_scan.py`: per-symbol signal + 0–100 confidence, breakout boost verified at +20, S/R + signals persisted, no orders). Phase 3 (Execution + Initial TP/SL) passed its green-light on 2026-05-30 via a forced paper entry (`scripts/run_trade_cycle --force BTC/USD`): BTC/USD position #1 recorded with correct size (qty 0.06461547, risk 0.40% at the conf-60 floor), 2R TP, price-SL, and support-break trigger; the Alpaca order read back FILLED with matching qty + avg price. **Phase 4 (Position Manager) is the active phase.** Two carry-overs from the green-light: (a) position #1 is still OPEN and unmanaged on paper (synthetic `--force` test — consider flattening before the soak), and (b) Alpaca takes crypto fees in the BASE asset, so the wallet qty is slightly below the recorded filled qty — Phase 4 should reconcile DB qty against the wallet / record fees for accurate exits + P&L.**
...
- **Applying the SQL schema programmatically:** the `db/*.sql` files use `GO` batch separators (an SSMS directive, not T-SQL). pyodbc/SQLAlchemy can't run `GO` — use `scripts/apply_sql.py` (splits on `^\s*GO\s*$` and runs each batch via `exec_driver_sql`, so `:r`/`:n` in comments aren't mistaken for binds), or run the file in SSMS. All three schema files are applied to `CryptoBotV2`; each is re-runnable / idempotent (guarded `CREATE`s + `MERGE` seeds).
...
.\.venv\Scripts\python.exe -m scripts.run_trade_cycle --dry-run  # Phase 3 cycle: size + report, NO orders
.\.venv\Scripts\python.exe -m scripts.run_trade_cycle            # Phase 3 cycle: places paper orders on a signal
.\.venv\Scripts\python.exe -m scripts.run_trade_cycle --force BTC/USD  # DEV: force an entry (Phase 3 green-light)
.\.venv\Scripts\python.exe -m scripts.apply_sql db/03_schema_phase3.sql  # apply a .sql file (splits on GO)
```
