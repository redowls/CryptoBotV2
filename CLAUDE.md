...
**Phases 1, 2, 3, and 4 are built and green-lit (Phases 1–2 on 2026-05-29; Phases 3–4 on 2026-05-30).** Phase 4's offline logic green-light passed (16/16) and it ran clean against live data + DB in dry-run; the one remaining Phase 4 item is a LIVE paper liquidation that writes `trade_history` (see TODO.md). The repo holds working code plus the design docs:
...
**Phases 5–8 are not built yet.** The modules they call for (`ai_*.py`) and the `heartbeat` table **do not exist** — search/verify before referencing them. `trade_history` is written by the Phase 4 Position Manager on position close, so it stays EMPTY until a position actually closes (pos#1 is still open on paper as of the Phase 4 green-light — and roughly flat, so nothing has tripped a trigger yet).
...
- **`core/orchestrator.py`** — `run_cycle(*, symbols=None, dry_run=False, force=False) -> int`: one hourly cycle. **EXIT pass first** (`manage_open_positions`, wrapped so one symbol's failure doesn't abort the cycle), THEN the ENTRY pass. Per symbol: fetch closed bars → upsert → detect+persist S/R → `evaluate` → persist signal; then enter (skip if held / no signal / below floor / in cool-down → `compute_size` → `place_entry` → `record_position`). Tracks open-position count, aggregate open risk, and cash *across* the cycle; the entry-side state is read AFTER the exit pass, so it reflects positions closed this bar. `dry_run` sizes + reports but places no orders; `force` (DEV) synthesizes a BUY setup (keeps real hints) to exercise an entry. TP = `entry_hint + tp.r_multiple * (entry_hint − stop_hint)`.
...
.\.venv\Scripts\python.exe -m scripts.apply_sql db/04_schema_phase4.sql  # apply a .sql file (splits on GO)
```
